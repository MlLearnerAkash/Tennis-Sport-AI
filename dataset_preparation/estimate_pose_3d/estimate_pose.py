...
def estimate_pose_3d(video_name, out_dir, save_video=False, device="cuda:0"):
    ...
    savepath = f"./{out_dir}/res_{video_basename}.mp4"
    savepath2d = f"./{out_dir}/res_2d_{video_basename}.mp4"
    ...
    os.system(
        f"ffmpeg -i {video_name} {out_dir}/raw_images/{video_basename}-%06d.png -loglevel quiet"
    )  # break video into frames

    files = os.listdir(f"{out_dir}/raw_images")
    files.sort()
    ...
    for file in tqdm(files):
        if not os.path.isdir(file) and file[-4:] in [".jpg", ".png"]:
            ...
            img_path = os.path.join(out_dir, "raw_images", file)
    ...
    # Here, we have all of the frames of the video, and iterate over them to run
    # the model
    ...
    for img_path in tqdm(img_path_list):
        ...
        with torch.no_grad():
            ...
            pred_xyz_jts_17 = (
                pose_output.pred_xyz_jts_17.reshape(17, 3).cpu().data.numpy()
            )
            ...
            pred_xyz_jts_24_struct = (
                pose_output.pred_xyz_jts_24_struct.reshape(24, 3).cpu().data.numpy()
            )
    ...
    res_filename = os.path.join(out_dir, f"res_{video_basename}.pk")
    with open(res_filename, "wb") as fid:
        pk.dump(res_db, fid)
